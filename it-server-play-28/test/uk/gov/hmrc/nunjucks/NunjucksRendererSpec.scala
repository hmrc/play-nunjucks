  ...
  "NunjucksRenderer" - {
    ...
    "fail to render in non-Dev modes" - {
      ...
      "given a template with a syntax error" in {
        ...
        whenReady(renderer.render("syntax-error.njk").failed) { case exception: PlayException.ExceptionSource =>
      ...
      "given a template with a missing import" in {
        ...
        whenReady(renderer.render("import-error.njk").failed) { case exception: PlayException =>
      ...
      "given a template with a javascript error" in {
        ...
        whenReady(renderer.render("javascript-error.njk").failed) { case exception: PlayException =>
      ...
      "given a template with a javascript error" in {
        ...
        result.toString() must (
          include("Template render error: (javascript-error.njk)")
            and include(
              "Error: Unable to call `routes[&quot;controllers&quot;][&quot;MissingController&quot;][&quot;get&quot;]`, which is undefined or falsey"
            )
        )
      }
    }
